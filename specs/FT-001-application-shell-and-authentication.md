...
| **PRD Reference**       | Section 11 — Security Constraints; Section 10 — Roles & Permissions; System Administration bounded context (authentication)  |
| **Open Questions**      | 4                                                                                                                              |
...
Users need secure, role-based access to the BST system with a consistent navigation experience. The legacy system requires Windows Authentication integration and maintains role-based permissions that control what functionality each user can access. Without a proper authentication foundation and navigation shell, users cannot access system features securely or efficiently navigate between different functional areas.

## 2. Benefit Hypothesis

We believe that providing a secure authentication system with role-based access control and intuitive navigation will result in streamlined user access and improved security posture for APHA staff. We will know this is true when users can authenticate seamlessly using their Windows credentials, access only the functionality appropriate to their role, and navigate efficiently between system areas without confusion.
...
| --- | -------------------------------------------- | ----------------------------------------------------------------- |
| 1   | Access the system securely using Windows credentials | User logs in automatically without additional credential prompts |
...
| 5   | Maintain secure session throughout work | System tracks user activity and maintains appropriate session security |
...
- Password management — delegated to Active Directory
...
- Integration with Active Directory is limited to authentication and user identity, not full directory services

## 6. User Stories and Acceptance Criteria

### US-001: User Authentication via Windows Authentication

**Story:** As an APHA staff member, I want to authenticate using my Windows credentials, so that I can access the BST system securely without managing separate login details.
...
```
┌─────────────────────────────────────────────────────────────────┐
│ BST System - Authentication                                     │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ╔════════════════════════════════════════════════════════════╗ │
│  ║                Authentication Required                     ║ │
│  ║                                                            ║ │
│  ║  The Brainstem Training System requires Windows           ║ │
│  ║  Authentication to access system functions.               ║ │
│  ║                                                            ║ │
│  ║  Please ensure you are logged into your Windows domain    ║ │
│  ║  account and try accessing the system again.              ║ │
│  ║                                                            ║ │
│  ║                     [ Try Again ]  [1]                    ║ │
│  ║                                                            ║ │
│  ╚════════════════════════════════════════════════════════════╝ │
...
[1] Try Again button - redirects to home page with authentication check
...
```gherkin
Scenario: Successful Windows Authentication
  Given I am logged into my Windows domain account
  And I have valid BST system permissions
  When I navigate to the BST system URL
  Then I am automatically authenticated without additional prompts
  And I am redirected to the home page
  And my user identity is established in the system session

Scenario: Failed Windows Authentication
  Given I am not logged into a Windows domain account
  When I navigate to the BST system URL
  Then I see an authentication required page
  And I am prompted to log into my Windows account
  And system access is denied until authentication succeeds

Scenario: Unauthorised User Access
  Given I am authenticated via Windows Authentication
  ...
  And system administrator is notified of the access attempt
...
║ ├─────────────────────────────────────────────────────────────┤   ║
║ │ 31/03 14:23   │ APHA\jsmith │ Login        │ Successful     │   ║
║ │ 31/03 14:22   │ APHA\bwilson│ Access Denied│ No permissions │   ║
║ │ 31/03 14:20   │ APHA\jdoe   │ Login        │ Successful     │   ║
║ │ 31/03 14:15   │ APHA\mchen  │ Logout       │ Session ended  │   ║
║ │ 31/03 14:10   │ APHA\jsmith │ Screen Access│ Training Recs  │   ║
║ └─────────────────────────────────────────────────────────────┘   ║
║                                                                   ║
...
Scenario: Authentication event logging
  ...
  Then an audit log entry is created with user ID, timestamp, authentication method, and result (success/failure)
...
2. IIS server challenges for Windows Authentication credentials
3. User's Windows session provides credentials automatically (or prompts if needed)
4. System validates credentials against Active Directory
...
**Decision points:**
- Authentication success/failure determines whether user proceeds or receives login prompt
...
- Failed authentication returns to authentication prompt or error page
- Unauthorised users are redirected to access denied page with support contact information

**Error/exception paths:**
- Active Directory unavailable: display technical error message with support contact
...
- **Purpose:** Displayed when Windows Authentication is required
- **Navigation context:** Pre-authentication state, no system navigation available
- **Layout:** Centred message panel on plain background
- **Components:**
  - Message panel with authentication instructions
  - "Try Again" button to retry authentication
...
| ------- | ---------------------------------------------- | -------------------------------------------------- | ----------------------------------------------------------------------------- |
| BR-001  | Windows Authentication is mandatory for all system access | All system entry points | If authentication fails, redirect to authentication required page with retry option |
...
|--------|-----------------|-----------|-------------|-------------|
| Windows Active Directory | Authentication API | Inbound | User authentication and identity validation using Windows Authentication protocols | Required |
...
| IIS Windows Authentication | Web server integration | Inbound | Integrated Windows Authentication configuration for seamless user credential handling | Required |
| SQL Server Database | Database connection | Bidirectional | User permission validation, audit logging, and session management data storage | Required |

**Active Directory Integration Details:**
- Uses Windows Authentication mechanism built into IIS
- Validates user credentials without requiring separate login interface
- Retrieves user identity in DOMAIN\username format
- Does not require complex Active Directory queries beyond authentication
...
| ------- | --------------------------------------------------------------------------- | -------------------------- | ------------------------------------------ |
| NFR-001 | Usability | Authentication should be seamless for domain users | No additional login prompts for authenticated Windows users |
...
| 3 | Database passwords stored in plain text configuration files | Security vulnerability exposing credentials | Use integrated authentication and encrypted connection strings | Eliminates credential exposure risk |
...
|------------|------|-------------|----------------------|
| SQL Server Database | Blocks | User permission validation, audit logging, and session data storage | Complete system failure - no user access possible |
| IIS Web Server | Blocks | Windows Authentication integration and web application hosting | Complete system failure - application inaccessible |
| .NET Framework | Blocks | Application runtime environment and Windows Authentication APIs | Complete system failure - application cannot run |
...
| ----------------------------------------------------------------- | ---------------------------- | ------------------------ | ---------------------------------- |
| Active Directory user account configuration | IT Infrastructure Team | BST system user accounts must be created and maintained in Active Directory | Pending |
| Database user permissions setup | Database Administrator | User permission records must be configured in BST database User and DataEntry tables | Pending |
| SMTP server configuration | IT Infrastructure Team | Email server must be configured for automated notifications | Pending |
| Security policy approval for authentication method | Information Security Team | Windows Authentication approach must be approved for BST system | Pending |
...
|---|-----------|-----------------|
| 1 | All BST users are members of Windows domain with valid Active Directory accounts | Would require implementing alternative authentication mechanism |
...
| --------------------------------------------- | -------------------------------------- | ----------------------------- | --------------------------- |
| Authentication time | N/A (manual assessment) | Under 2 seconds for domain users | Page load time measurement |
...
| Authentication-related support calls | Unknown baseline | Reduce by 80% through seamless authentication | Support ticket analysis |
...
| ---------------- | -------------- | ------------------------------------------ |
| **Human Effort** | 15 person-days | Assumes existing Windows Authentication infrastructure, straightforward database integration, standard web forms development patterns, and minimal custom security components required |
...
| Term | Definition |
|------|-----------|
| **Windows Authentication** | Authentication mechanism that uses the user's Windows domain credentials to grant access to web applications without requiring separate login |
