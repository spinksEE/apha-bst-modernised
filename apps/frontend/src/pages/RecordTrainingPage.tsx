...
export function RecordTrainingPage(): React.JSX.Element {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const preselectedTraineeId = searchParams.get('trainee_id');
  ...
  const [traineeId, setTraineeId] = useState<number | null>(
    preselectedTraineeId ? parseInt(preselectedTraineeId, 10) : null,
  );
  ...
  // Pre-select trainee from query param
  useEffect(() => {
    if (preselectedTraineeId) {
      const id = parseInt(preselectedTraineeId, 10);
      if (!isNaN(id)) {
        getPersons(undefined, undefined)
          .then((persons) => {
  ...
  function handleTraineeSelect(personIdStr: string) {
    ...
    const person = personResults.find((p) => p.person_id === id);
    if (person) {
      setTraineeId(person.person_id);
      setTraineeSearchValue(person.display_name);
    }
  ...
  function handleTrainerSelect(trainerIdStr: string) {
    const id = parseInt(trainerIdStr, 10);
